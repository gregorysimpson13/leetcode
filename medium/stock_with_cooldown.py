...
[1,2,3,0,2]
 ...
 # initialize max_diff to -prices[0], curr_prof to 0, prev_prof = 0
 # curr_prof = max(curr_prof, prices[i] + max_diff)
...
# class Solution:
#     def maxProfit(self, prices: List[int]) -> int:
#         def calc_profit(index, profit, brought):
#             if index >= len(prices):
#                 return profit
#             if brought == None:
#                 return calc_profit(index+1, profit, prices[index])
#             return max(calc_profit(index+1, profit, brought), calc_profit(index+1, profit, prices[index]), calc_profit(index+2, profit + (prices[index] - brought), None))
#         return calc_profit(1,0,prices[0])
...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        def calc_profit(index, profit, brought):
            if index >= len(prices):
                return profit
            if (index, profit) in cache:
                return cache[(index, profit)]
            max_profit = 0
            if brought == None:
                max_profit = calc_profit(index+1, profit, prices[index])
            else:
                max_profit = max(calc_profit(index+1, profit, brought), calc_profit(index+1, profit, prices[index]), calc_profit(index+2, profit + (prices[index] - brought), None))
            cache[(index,profit)] = max_profit
            return max_profit
        return calc_profit(1,0,prices[0])


class LeetcodeSolution1:
    # https://leetcode.com/problems/best-time-to-buy-and-sell-stock-with-cooldown/discuss/685572/Simple-DP-Solution
    def maxProfit(self, prices: List[int]) -> int:
        if len(prices) <= 1: return 0
        profits = [0] * len(prices)
        profits[0], profits[1] = 0, max(0, prices[1]-prices[0])
        max_diff = max(-prices[0], -prices[1])
        for i in range(1, len(prices)):
            profits[i] = max(profits[i-1], prices[i] + max_diff)
            max_diff = max(max_diff, profits[i-2] - prices[i])
        return profits[-1]

class LeetcodeSolution2:
    # https://leetcode.com/problems/best-time-to-buy-and-sell-stock-with-cooldown/discuss/720091/Python3-dp-solution-Best-Time-to-Buy-and-Sell-Stock-with-Cooldown
    def maxProfit(self, prices: List[int]) -> int:
...
print(Solution1().maxProfit([1,2,3,0,2]), 3, LeetcodeSolution1().maxProfit([1,2,3,0,2]))
print(Solution1().maxProfit([1,2,3,0,2,5]), 6, LeetcodeSolution1().maxProfit([1,2,3,0,2,5]))
print(Solution1().maxProfit([0,1,2,3,4,5]), 5, LeetcodeSolution1().maxProfit([0,1,2,3,4,5]))
print(Solution1().maxProfit([0,1,2,3,4,10]), 10, LeetcodeSolution1().maxProfit([0,1,2,3,4,10]))
